...
from ...services import ExecutionService, OrchestratorService, AssignmentService

try:
    from ...worker import submit_task_to_queue, TaskQueue
...
async def submit_task_to_worker(
    ...
    if not WORKER_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Worker queue not available. Redis is required."
        )
...
async def orchestrator_process_task(
    ...
    from ...models.enums import PriorityEnum, DomainEnum
