...
TOOL_CONFIG_SCHEMAS: dict = {
    "web_browser": {
        ...
        "fields": [
            ...
            {"name": "max_length", "label": "Max Content Length (chars)", "type": "number", "default": 10000},
            {"name": "user_agent", "label": "Custom User Agent", "type": "text", "default": ""},
        ],
    },
    "web_search": {
        "label": "Web Search",
        ...
        "category": "web",
        "fields": [
            {"name": "region", "label": "Region", "type": "text", "default": "wt-wt",
             "description": "e.g. wt-wt (global), us-en, es-es"},
            {"name": "max_results", "label": "Default Max Results", "type": "number", "default": 10},
            {"name": "safe_search", "label": "Safe Search", "type": "select",
             "default": "moderate", "options": ["on", "moderate", "off"]},
        ],
    },
    "browser_automation": {
        ...
        "fields": [
            ...
            {"name": "timeout", "label": "Default Timeout (ms)", "type": "number", "default": 30000},
        ],
    },
    "http_api": {
    ...
    "system_shell": {
        ...
        "fields": [
            {"name": "timeout", "label": "Timeout (seconds)", "type": "number", "default": 30},
            {"name": "allowed_commands", "label": "Allowed Commands (comma-separated)",
             "type": "text", "default": "ls,cat,grep,find,echo",
    ...
    "filesystem": {
        ...
        "category": "file",
        "fields": [
            {"name": "workspace_dir", "label": "Workspace Directory",
             "type": "text", "default": "/workspace/files",
    ...
    "code_executor": {
        ...
        "category": "code",
        "fields": [
            {"name": "default_timeout", "label": "Default Timeout (seconds)", "type": "number", "default": 30},
            {"name": "max_timeout", "label": "Max Allowed Timeout (seconds)", "type": "number", "default": 120},
        ],
    },
    "document_reader": {
        ...
        "category": "file",
        "fields": [
            {"name": "max_chars", "label": "Max Characters to Extract", "type": "number", "default": 50000},
        ],
    },
    "database_query": {
        ...
        "fields": [
            ...
            {"name": "database_url", "label": "Custom Database URL (optional)",
             "type": "password", "default": "", "env_var": "DATABASE_URL",
    ...
    "agent_memory": {
        ...
        "category": "data",
        "fields": [
            {"name": "openai_api_key", "label": "OpenAI API Key (for embeddings)",
             "type": "password", "default": "", "env_var": "OPENAI_API_KEY",
    ...
    "github": {
        ...
        "fields": [
            {"name": "token", "label": "Personal Access Token", "type": "password",
             "default": "", "env_var": "GITHUB_TOKEN", "required": True,
             "description": "Generate at github.com/settings/tokens (needs repo scope)"},
            {"name": "default_owner", "label": "Default Owner / Organization",
             "type": "text", "default": ""},
        ],
    },
    "email": {
        ...
        "fields": [
            {"name": "smtp_host", "label": "SMTP Host", "type": "text", "default": "",
             "env_var": "EMAIL_SMTP_HOST", "description": "e.g. smtp.gmail.com"},
            {"name": "smtp_port", "label": "SMTP Port", "type": "number", "default": 587, "env_var": "EMAIL_SMTP_PORT"},
            {"name": "smtp_user", "label": "SMTP Username / From Address",
             "type": "text", "default": "", "env_var": "EMAIL_SMTP_USER"},
            {"name": "smtp_password", "label": "SMTP Password / App Password",
             "type": "password", "default": "", "env_var": "EMAIL_SMTP_PASSWORD"},
            {"name": "imap_host", "label": "IMAP Host (for reading email)",
             "type": "text", "default": "", "env_var": "EMAIL_IMAP_HOST",
             "description": "e.g. imap.gmail.com — leave blank if same as SMTP host"},
            {"name": "imap_port", "label": "IMAP Port", "type": "number", "default": 993, "env_var": "EMAIL_IMAP_PORT"},
        ],
    },
    "notification": {
        "label": "Notifications",
        ...
        "category": "communication",
        "fields": [
            {"name": "slack_webhook", "label": "Slack Incoming Webhook URL",
             "type": "password", "default": "", "env_var": "SLACK_WEBHOOK_URL",
             "description": "api.slack.com/apps → Your App → Incoming Webhooks"},
            {"name": "discord_webhook", "label": "Discord Webhook URL",
             "type": "password", "default": "", "env_var": "DISCORD_WEBHOOK_URL",
             "description": "Channel Settings → Integrations → Webhooks"},
            {"name": "teams_webhook", "label": "Microsoft Teams Connector URL",
             "type": "password", "default": "", "env_var": "TEAMS_WEBHOOK_URL"},
        ],
    },
    "docs_search": {
        ...
        "category": "web",
        "fields": [
            {"name": "context7_token", "label": "Context7 API Token (optional)",
             "type": "password", "default": "", "env_var": "CONTEXT7_TOKEN",
...
def _build_tool_response(
    tool_name: str, db_record: IntegrationConfig | None
) -> dict:
    schema = TOOL_CONFIG_SCHEMAS.get(tool_name, {})
...
async def list_tool_configs(
    ...
    records: dict[str, IntegrationConfig] = {r.tool_name: r for r in result.scalars().all()}
...
async def test_tool_config(
    ...
    try:
        ...
        if tool_name == "github":
            ...
            return {"data": {"success": True, "message": "Webhook URLs saved — send a test message via agents to verify delivery."}}
        elif tool_name == "email":
            return {"data": {"success": True, "message": "Email config saved — send a test email via agents to verify SMTP."}}
        ...
        return {
            "data": {
                ...
                "output": (result.stdout or str(result.data))[:500] if result.success else result.error,
...
async def update_mcp_server(
    ...
    server = await _get_server_or_404(session, server_id)
    updatable = ["name", "description", "server_type", "url", "command",
                 "args", "env_vars", "headers", "enabled"]
...
async def test_mcp_server(
    ...
    try:
        ...
        await session.commit()
        await session.refresh(server)
        return {"data": {"status": "connected", "tools": tools, "tool_count": len(tools)}}
    except Exception as e:
        server.status = "error"
