...
from app.core.realtime import broadcast_activity, broadcast_metrics, EventType
...
async def _get_default_llm_config_id(session: AsyncSession) -> Optional[UUID]:
...
async def chat(
    ...
    try:
        ...
        if parent_task_id:
            try:
                task_id = int(parent_task_id.split("-")[0]) if isinstance(parent_task_id, str) else int(parent_task_id)
