...
from app.models.config import ConfigCategory, ConfigPreset, ConfigValueType
...
async def get_all_configuration(
    ...
    include_secrets: bool = Query(
        False, description="Include secret values (admin only)"
    ),
...
async def get_configuration_value(
    key: str, service: ConfigService = Depends(get_config_service)
):
...
async def create_configuration(
    ...
    value_type: ConfigValueType = Body(
        default=ConfigValueType.STRING, description="Value type"
    ),
    category: ConfigCategory = Body(
        default=ConfigCategory.GENERAL, description="Category"
    ),
    ...
    updated_by: str | None = Body(default=None, description="Updated by user"),
    service: ConfigService = Depends(get_config_service),
):
...
async def update_configuration(
    ...
    updated_by: str | None = Body(default=None, description="Updated by user"),
    change_reason: str | None = Body(default=None, description="Reason for change"),
    ...
    try:
        config = await service.update(
            key, value, updated_by, change_reason, skip_validation
        )
...
async def bulk_update_configuration(
    ...
    updated_by: str | None = Body(default=None, description="Updated by user"),
    change_reason: str | None = Body(default=None, description="Reason for changes"),
    service: ConfigService = Depends(get_config_service),
):
...
async def import_configuration(
    ...
    overwrite_existing: bool = Body(
        default=True, description="Overwrite existing values"
    ),
    skip_validation: bool = Body(default=False, description="Skip validation"),
    updated_by: str | None = Body(default=None, description="Imported by user"),
    service: ConfigService = Depends(get_config_service),
):
    """Import configuration from JSON"""
    try:
        results = await service.import_config(
            data, updated_by, overwrite_existing, skip_validation
        )
...
async def list_presets(
    category: str | None = None, service: ConfigService = Depends(get_config_service)
):
...
async def apply_preset(
    preset_id: UUID,
    updated_by: str | None = Body(default=None, description="Applied by user"),
    service: ConfigService = Depends(get_config_service),
):
    """Apply a configuration preset"""
    try:
        await service.apply_preset(preset_id, updated_by)
        return {
            "success": True,
...
async def get_statistics(service: ConfigService = Depends(get_config_service)):
...
async def validate_configuration(
    ...
    try:
        ...
        return {"valid": is_valid, "key": key, "value": value, "error": error_msg}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def validate_bulk_configuration(
    ...
    try:
        ...
        for key, value in updates.items():
            is_valid, error_msg = await service.validate_config(key, value)
            results[key] = {"valid": is_valid, "error": error_msg}
...
async def get_llm_providers_status():
    ...
    provider_configs = [
        ...
        (
            "Anthropic",
            "ANTHROPIC",
            env_settings.ANTHROPIC_API_KEY,
...
async def get_messaging_status():
    ...
    return {
        "success": True,
        "public_domain": env_settings.PUBLIC_DOMAIN
        or "Not set - required for webhooks",
        "platforms": {
            "telegram": {
                "name": "Telegram",
                ...
                "webhook_url": env_settings.TELEGRAM_WEBHOOK_URL
                or (
                    f"{env_settings.PUBLIC_DOMAIN}/api/v1/telegram/webhook"
                    if env_settings.PUBLIC_DOMAIN
                    else None
                ),
            ...
            "discord": {
                ...
                "configured": env_settings.discord_enabled
                and bool(env_settings.DISCORD_APPLICATION_ID),
            ...
            "slack": {
                ...
                "configured": env_settings.slack_enabled
                and bool(env_settings.SLACK_SIGNING_SECRET),
            ...
            "whatsapp": {
                ...
                "phone_number_id_configured": bool(
                    env_settings.WHATSAPP_PHONE_NUMBER_ID
                ),
                "business_account_configured": bool(
                    env_settings.WHATSAPP_BUSINESS_ACCOUNT_ID
                ),
...
async def get_feature_flags(service: ConfigService = Depends(get_config_service)):
    ...
    try:
        ...
        return {
            ...
            "features": features,
            "total_enabled": sum(
                1 for f in features.values() if f.get("value", False) is True
            ),
        }
    except Exception as e:
        raise HTTPException(
