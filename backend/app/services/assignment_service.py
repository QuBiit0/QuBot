...
from ..models.enums import TaskStatusEnum, TaskEventTypeEnum, DomainEnum, AgentStatusEnum
...
class AssignmentService:
    ...
    def _calculate_domain_match(
        ...
        related_domains = {
            ...
            DomainEnum.DATA: [DomainEnum.SOFTWARE, DomainEnum.RESEARCH, DomainEnum.FINANCE],
            DomainEnum.FINANCE: [DomainEnum.DATA, DomainEnum.LEGAL, DomainEnum.OPERATIONS],
    ...
    async def _calculate_performance_score(
        ...
        result = await self.session.execute(
            select(TaskEvent).join(Task).where(
                Task.assigned_agent_id == agent.id,
    ...
    async def rebalance_workload(
        ...
        for task in active_tasks:
            ...
            if task.assigned_agent_id and current_assignment.agent_id != task.assigned_agent_id:
                current_agent = await self.agent_service.get_agent(task.assigned_agent_id)
