...
class FilesystemTool(BaseTool):
    ...
    def _get_parameters_schema(self) -> Dict[str, ToolParameter]:
        return {
            "action": ToolParameter(
                ...
                required=True,
                enum=["read", "write", "append", "delete", "list", "mkdir", "rmdir", "exists", "info"],
            ),
            "path": ToolParameter(
                name="path",
                type="string",
    ...
    async def _action_write(self, path: Path, content: Optional[str], encoding: str) -> ToolResult:
        """Write file content"""
        if content is None:
    ...
    async def _action_append(self, path: Path, content: Optional[str], encoding: str) -> ToolResult:
        """Append to file"""
        if content is None:
        ...
        if path.exists() and not self._validate_write_extension(path):
            return ToolResult(
                success=False,
                error=f"File extension not allowed",
            )
    ...
    async def _action_delete(self, path: Path) -> ToolResult:
        ...
        if path.is_dir():
            return ToolResult(success=False, error=f"Use rmdir action to delete directories")
    ...
    async def _action_list(self, path: Path, recursive: bool) -> ToolResult:
        ...
        if not path.is_dir():
            return ToolResult(success=False, error=f"Path is not a directory: {path.name}")
