...
from .base import BaseLlmProvider, LlmResponse, ToolCall, ToolDefinition, FinishReason
...
class GoogleProvider(BaseLlmProvider):
    ...
    async def complete(
        ...
        for candidate in response.candidates:
            for part in candidate.content.parts:
                if part.text:
                    ...
                    tool_calls.append(ToolCall.from_google({
        ...
        if response.candidates:
            ...
            finish_reason = finish_reason_map.get(finish_reason_code, FinishReason.ERROR)
        ...
        return LlmResponse(
            ...
            raw_response={
                "candidates": [
                    {
                        "content": {
                            "parts": [{"text": p.text} if p.text else {"function_call": {"name": p.function_call.name, "args": dict(p.function_call.args)}} for p in c.content.parts],
                            "role": c.content.role,
                        },
                        "finish_reason": c.finish_reason.name if c.finish_reason else None,
                    }
