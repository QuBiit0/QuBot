...
class FilesystemTool(BaseTool):
    name = "filesystem"
    description = "Permite leer y escribir archivos en el sistema local de forma segura."
    parameters = {
        "type": "object",
        "properties": {
            "operation": {"type": "string", "enum": ["read", "write", "list"]},
            "path": {"type": "string"},
            "content": {"type": "string", "description": "Contenido para la operación de escritura"}
        },
    ...
    async def execute(self, operation: str, path: str, content: str = None) -> ToolResult:
        try:
            ...
            if not full_path.startswith(os.path.abspath(safe_base)):
                return ToolResult(success=False, output=None, error="Acceso denegado: Path fuera del sandbox.")
            ...
            return ToolResult(success=False, output=None, error="Operación no soportada.")
