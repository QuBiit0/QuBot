...
class SystemConfig(SQLModel, table=True):
    ...
    value_string: Optional[str] = Field(default=None, sa_column=Column(Text))
    value_integer: Optional[int] = Field(default=None)
    value_float: Optional[float] = Field(default=None)
    value_boolean: Optional[bool] = Field(default=None)
    value_json: Optional[dict] = Field(default=None, sa_column=Column(JSON))
    
    # Metadata
    description: str = Field(default="", max_length=500)
    is_editable: bool = Field(default=True)
    is_secret: bool = Field(default=False)  # If True, value is masked in API responses
    requires_restart: bool = Field(default=False)  # If True, requires app restart to take effect
    ...
    def set_value(self, value: Any, value_type: Optional[ConfigValueType] = None):
...
class ConfigPreset(SQLModel, table=True):
    ...
    # Category
    category: str = Field(default="custom", max_length=50)  # development, production, testing, custom
...
class EnvironmentConfig(SQLModel, table=True):
    ...
    environment: str = Field(max_length=50, index=True)  # development, staging, production
