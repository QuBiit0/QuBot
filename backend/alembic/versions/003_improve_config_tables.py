...
def upgrade() -> None:
    ...
    try:
        op.add_column(
            "system_config", sa.Column("validation_rules", sa.JSON(), nullable=True)
        )
    except:
        pass
    ...
    try:
        op.create_index(
            "idx_system_config_updated", "system_config", ["updated_at"], unique=False
        )
    except:
        pass
    ...
    try:
        op.create_index(
            "idx_config_history_changed", "config_history", ["changed_at"], unique=False
        )
    except:
        pass
