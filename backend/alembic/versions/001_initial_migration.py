...
def upgrade() -> None:
    ...
    op.create_table('agent_class',
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('name', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('description', sqlmodel.sql.sqltypes.AutoString(length=500), nullable=False),
    sa.Column('domain', sa.Enum('TECH', 'BUSINESS', 'FINANCE', 'HR', 'MARKETING', 'LEGAL', 'PERSONAL', 'OTHER', name='domainenum'), nullable=False),
    ...
    op.create_index(op.f('ix_agent_class_domain'), 'agent_class', ['domain'], unique=False)
    ...
    sa.Column('embedding_ref', sqlmodel.sql.sqltypes.AutoString(length=200), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    ...
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('name', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('provider', sa.Enum('OPENAI', 'ANTHROPIC', 'GOOGLE', 'GROQ', 'OPENROUTER', 'DEEPSEEK', 'KIMI', 'MINIMAX', 'ZHIPU', 'LOCAL', 'CUSTOM', 'OTHER', name='llmproviderenum'), nullable=False),
    ...
    sa.Column('api_key_ref', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('extra_config', sa.JSON(), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_llm_config_provider'), 'llm_config', ['provider'], unique=False)
    ...
    sa.Column('name', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('description', sqlmodel.sql.sqltypes.AutoString(length=500), nullable=False),
    sa.Column('type', sa.Enum('SYSTEM_SHELL', 'WEB_BROWSER', 'FILESYSTEM', 'HTTP_API', 'SCHEDULER', 'CUSTOM', name='tooltypeenum'), nullable=False),
    ...
    op.create_table('messaging_channel',
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('platform', sa.Enum('TELEGRAM', 'WHATSAPP', 'DISCORD', 'SLACK', name='messagingplatformenum'), nullable=False),
    ...
    op.create_index('idx_messaging_channel_platform', 'messaging_channel', ['platform'], unique=False)
    ...
    sa.Column('status', sa.Enum('BACKLOG', 'IN_PROGRESS', 'IN_REVIEW', 'DONE', 'FAILED', name='taskstatusenum'), nullable=False),
    sa.Column('priority', sa.Enum('LOW', 'MEDIUM', 'HIGH', 'CRITICAL', name='priorityenum'), nullable=False),
    sa.Column('domain_hint', sa.Enum('TECH', 'BUSINESS', 'FINANCE', 'HR', 'MARKETING', 'LEGAL', 'PERSONAL', 'OTHER', name='domainenum'), nullable=True),
    ...
    op.create_index('idx_task_domain_status', 'task', ['domain_hint', 'status'], unique=False)
    op.create_index('idx_task_status_agent', 'task', ['status', 'assigned_agent_id'], unique=False)
    op.create_index(op.f('ix_task_assigned_agent_id'), 'task', ['assigned_agent_id'], unique=False)
    ...
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('name', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('gender', sa.Enum('MALE', 'FEMALE', 'NON_BINARY', name='genderenum'), nullable=False),
    sa.Column('class_id', sa.Uuid(), nullable=False),
    sa.Column('domain', sa.Enum('TECH', 'BUSINESS', 'FINANCE', 'HR', 'MARKETING', 'LEGAL', 'PERSONAL', 'OTHER', name='domainenum'), nullable=False),
    sa.Column('role_description', sqlmodel.sql.sqltypes.AutoString(length=500), nullable=False),
    ...
    sa.Column('status', sa.Enum('IDLE', 'WORKING', 'ERROR', 'OFFLINE', name='agentstatusenum'), nullable=False),
    sa.Column('current_task_id', sa.Uuid(), nullable=True),
    sa.Column('is_orchestrator', sa.Boolean(), nullable=False),
    sa.Column('last_active_at', sa.DateTime(), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['class_id'], ['agent_class.id'], ),
    ...
    op.create_foreign_key('fk_task_assigned_agent', 'task', 'agent', ['assigned_agent_id'], ['id'])
    
    # agent_tool - depends on agent and tool
    op.create_table('agent_tool',
    sa.Column('agent_id', sa.Uuid(), nullable=False),
    sa.Column('tool_id', sa.Uuid(), nullable=False),
    sa.Column('permission', sa.Enum('READ_ONLY', 'READ_WRITE', 'DANGEROUS', name='permissionenum'), nullable=False),
    sa.Column('config', sa.JSON(), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    ...
    op.create_index('idx_llm_log_agent_time', 'llm_call_log', ['agent_id', 'created_at'], unique=False)
    op.create_index('idx_llm_log_task', 'llm_call_log', ['task_id'], unique=False)
    op.create_index(op.f('ix_llm_call_log_agent_id'), 'llm_call_log', ['agent_id'], unique=False)
    op.create_index(op.f('ix_llm_call_log_task_id'), 'llm_call_log', ['task_id'], unique=False)
    ...
    op.create_index('idx_agent_memory_agent_importance', 'agent_memory', ['agent_id', 'importance'], unique=False)
    ...
    sa.Column('type', sa.Enum('CREATED', 'ASSIGNED', 'STARTED', 'TOOL_CALL', 'PROGRESS_UPDATE', 'COMPLETED', 'FAILED', 'COMMENT', name='taskeventtypeenum'), nullable=False),
    sa.Column('agent_id', sa.Uuid(), nullable=True),
    ...
    op.create_index('idx_task_event_task_time', 'task_event', ['task_id', 'created_at'], unique=False)
    ...
    op.create_index(op.f('ix_task_memory_task_id'), 'task_memory', ['task_id'], unique=False)
    
    # conversation - depends on messaging_channel
    op.create_table('conversation',
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('channel_id', sa.Uuid(), nullable=False),
    sa.Column('external_user_id', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('external_user_name', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('context', sa.JSON(), nullable=True),
    sa.Column('is_active', sa.Boolean(), nullable=False),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['channel_id'], ['messaging_channel.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_conversation_channel_user', 'conversation', ['channel_id', 'external_user_id'], unique=False)
    op.create_index(op.f('ix_conversation_channel_id'), 'conversation', ['channel_id'], unique=False)
    ...
    op.create_index('idx_conv_message_conv_time', 'conversation_message', ['conversation_id', 'created_at'], unique=False)
